...
set.seed(5L); d_sup <- d_sup[sample(nrow(d_sup), nrow(d_sup),replace = FALSE),]
...
# n <- d_sup[,.N]
fwrite(d_sup[ 1 : round(d_sup[,.N]*.1 -1)   ,.(tag,label_desc) ],file='FT/data/mlstr.test5',na = '',row.names = FALSE,col.names = FALSE,sep = '\t')
fwrite(d_sup[ round(d_sup[,.N]*.1) : round(d_sup[,.N]*.2 -1)   ,.(tag,label_desc) ],file='FT/data/mlstr.validation5',na = '',row.names = FALSE,col.names = FALSE,sep = '\t')
fwrite(d_sup[ round(d_sup[,.N]*.2) : round(d_sup[,.N])   ,.(tag,label_desc) ],file='FT/data/mlstr.train5',na = '',row.names = FALSE,col.names = FALSE,sep = '\t')
...
d_unsup <- fread('~/teradata/analysis/machine-learning/nlp/data/full/FULL_UNSUP.csv')
d_unsup <- d_unsup[sample(nrow(d_unsup), nrow(d_unsup),replace = FALSE),]
d_unsup[,label_desc:=clean_data(label_desc)]
d_unsup <- d_unsup[!duplicated(label_desc),]

#- save file 
fwrite(d_unsup[,.(label_desc)],file='~/teradata/analysis/machine-learning/nlp/document_classification/FT/data/unsup_csv/mlstr.unsup.wv.csv',na = '',row.names = FALSE,quote = FALSE,col.names = FALSE)
fwrite(d_unsup[,.(label_desc)],file='~/teradata/analysis/machine-learning/nlp/document_classification/FT/data/mlstr.wv.unsup',na = '',row.names = FALSE,quote = FALSE,col.names = FALSE)
